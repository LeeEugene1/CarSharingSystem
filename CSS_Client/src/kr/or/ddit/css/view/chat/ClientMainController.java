package kr.or.ddit.css.view.chat;
...
//    private Button btnJoin;
   ...
   // private String memId="";
    ...
    @FXML
    void initialize() {
       // tfIp.setText("192.168.203.36");
        chatIp="192.168.203.36";
        
       // tfPort.setText("8989");
        chatPort="8989";
        
        if(AdminLoginSession.adminSession==null) {
        	tfName.setText(LoginSession.session.getMem_id());
        	//memId=LoginSession.session.getMem_id();
        }else {
        	tfName.setText(AdminLoginSession.adminSession.getAdmin_id());	
        }
...
//			}
			...
			// 방법1
			clientSocket = new Socket(chatIp, Integer.parseInt(chatPort));
...
//			btnJoin.setText("접속 종료");
    ...
    public void msgSend(){
    	try {
            ...
            String sendMessage =  user + tfMsg.getText();
            byte[] byteArray = sendMessage.getBytes("UTF-8");
            OutputStream outputStream = clientSocket.getOutputStream();
            outputStream.write(byteArray);
        } catch (Exception e) {
...
//	           				btnJoin.setText("서버 접속");
                    ...
                    String ttt  = "";
                    if(temp[0].startsWith("관리자->")) {
                    	temp[0] = temp[0].substring("관리자->".length()); 
                    }
                    
                    if(LoginSession.session == null || LoginSession.session.getMem_id().equals(temp[0]) ) {
