...
public class carPairingUpdateController {
    ...
    void insertOk(ActionEvent event) throws IOException {
    	...
    	int board_id = CarPairingSession.carPairingSession;
    	String memId = LoginSession.session.getMem_id();
    	
    	CPIBVo.setPairinginsertboard_id(board_id);
    	...
    	int cnt = service.updateBoard(CPIBVo);
    	if(cnt>0) {
			...
			for(CarPairingInsertBoardVO bVO : list_edit) {
				data_edit.add(bVO);
			}
